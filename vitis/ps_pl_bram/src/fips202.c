...
// 添加下面这行，定义一个全局数组来存储拦截到的1600位state数据
uint8_t shake_input_state_to_pl[200];
...
void shake256_absorb(uint64_t *s, const uint8_t *input, size_t inlen) {
    ...
    // ==================== 在这里添加修改 ====================
        // 此时，`keccak_absorb` 已经执行完毕，s 数组已经包含了完整的1600位状态
        // 我们将它复制到之前定义的全局变量中，以便 main.c 可以访问它
        memcpy(shake_input_state_to_pl, s, 200);
        // ======================================================
}
